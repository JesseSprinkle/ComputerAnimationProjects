...
void init()
{
	...
	for(const auto &mesh : dataInput.meshData) {
		...
		{
			shape = make_shared<BlendShape>();
			shape->loadMesh(DATA_DIR + mesh[0]);
			shape->setTextureFilename(mesh[1]);
			for (const auto& blend_shape : dataInput.blendData)
			{
				((BlendShape*)shape.get())->addBlendShape(DATA_DIR + blend_shape);
			}
		}
		else
		{
	...
	prog->addAttribute("dBlendPos0");
	prog->addAttribute("dBlendPos1");
	prog->addAttribute("dBlendPos2");
	prog->addAttribute("dBlendNor0");
	prog->addAttribute("dBlendNor1");
	prog->addAttribute("dBlendNor2");
	prog->addUniform("P");
	prog->addUniform("MV");
	prog->addUniform("ka");
	prog->addUniform("ks");
	...
	prog->addUniform("numBlendShapes");
	prog->addUniform("t");
...
void render()
{
	...
	for(const auto &shape : shapes) {
		...
		glUniform1f(prog->getUniform("t"), t1);
...
void loadDataInputFile()
{
	...
	while(1) {
		...
		} else if (key.compare("HEAD_BLEND_SHAPE") == 0) {
			ss >> value;
			dataInput.blendData.push_back(value);
		} else {
