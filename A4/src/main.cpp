...
static void cursor_position_callback(GLFWwindow* window, double xmouse, double ymouse)
{
	...
	if(keyToggles[(unsigned)' ']) 
	{
		...
		VectorXd beforeAngles(NLINKS);
		for (int i = 0; i < NLINKS; i++)
		{
			beforeAngles(i) = links[i]->getAngle();
		}

		OptimizerGDLS oGDLS(10 * NLINKS);
		OptimizerNM oNM(10 * NLINKS);

		shared_ptr<Objective> objective = make_shared<Objective>(x);
		VectorXd gdlsAngles = oGDLS.optimize(objective, beforeAngles);
		VectorXd nmAngles = oNM.optimize(objective, gdlsAngles);

		VectorXd finalAngles;
		double nmF = objective->evalObjective(nmAngles);
		double gdlsF = objective->evalObjective(gdlsAngles);
		if (nmF < gdlsF)
		{
			finalAngles = nmAngles;
		}
		else
		{
			finalAngles = gdlsAngles;
		}
		cout << "Using " << ((nmF < gdlsF) ? "NM+GDLS" : "GDLS") << " Approximation" << endl;
		cout << "GDLS Iterations: " << oGDLS.getIter() << endl;
		cout << "NM Iterations: " << oNM.getIter() << endl << endl; 
		for (int i = 0; i < NLINKS; ++i) {
			links[i]->setAngle(finalAngles(i));
		}
...
static void init()
{
	...
	Eigen::Matrix4d trans = Matrix4d::Identity();
	trans(0, 3) = 0.5;
	for (int i = 0; i < NLINKS; i++)
	{
		links.push_back(make_shared<Link>());
		links[i]->setPosition(1.0f, 0.0f);
		links[i]->setMeshMatrix(trans);
	}
	links[0]->setPosition(0, 0);
	for (int i = 0; i < NLINKS - 1; i++)
	{
		links[i]->addChild(links[i + 1]);
	}
	...
	VectorXd g(4);
	MatrixXd h(4, 4);
	shared_ptr<Objective> objective = make_shared<Objective>(Vector2d(3, 1));
	objective->evalObjective(v, g, h);
